@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="transparent bg-white">
        <div class="nav-wrapper flex justify-between flex-1 sm:hidden">
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-green-500 bg-gray-400 border border-green-600 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
